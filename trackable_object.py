...
class TrackableObject:
    ...
    def __init__(self, objectID, centroid, label=label, color=None, last_rect=None, w=2000, h=2000):
    ...
    def add_centroid(self, centroid):
        if TEST_DEAD_RECKONING:
            backtrack = 12
            if self.lost_mode:
                if points_match(centroid, self.last_detected_center):
                ...
                if points_match(centroid, last_c) and points_match(centroid, last_c2) and len(self.centroids) > backtrack+3:
...
def bound_point(x_m, y_m, pt):
    new_x = bound(0, x_m, pt[0])
    new_y = bound(0, y_m, pt[1])
    return [new_x, new_y]
